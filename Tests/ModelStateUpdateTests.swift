...
final class ModelStateUpdateTests: XCTestCase {
  ...
  func testSectionReload() throws {
    let initialSections = ModelHelpers.basicSectionModels(
      numberOfSections: 1,
      numberOfItemsPerSection: 3
    )
    modelState.setSections(initialSections)
    let replacementSection = try XCTUnwrap(ModelHelpers.basicSectionModels(
      numberOfSections: 3,
      numberOfItemsPerSection: 1
    ).first)
    ...
    XCTAssert(
      try (
        modelStateBeforeBatchUpdates.indexPathForItemModel(
          ...
          modelStateBeforeBatchUpdates.indexPathForItemModel(
            withID: try XCTUnwrap(modelStateBeforeBatchUpdates.idForItemModel(at: IndexPath(item: 1, section: 0)))
          )! ==
          IndexPath(item: 1, section: 0) &&
          modelStateBeforeBatchUpdates.indexPathForItemModel(
            withID: try XCTUnwrap(modelStateBeforeBatchUpdates.idForItemModel(at: IndexPath(item: 0, section: 2)))
          )! ==
  ...
  func testAllUpdatesNoCrash() throws {
    let initialSections = ModelHelpers.basicSectionModels(
      numberOfSections: 6,
      numberOfItemsPerSection: 2
    )
    ...
    modelState.applyUpdates(
      [
        .sectionReload(
          sectionIndex: 3,
          newSection: try XCTUnwrap(ModelHelpers.basicSectionModels(
            numberOfSections: 1,
            numberOfItemsPerSection: 2
          ).first)
        ),
